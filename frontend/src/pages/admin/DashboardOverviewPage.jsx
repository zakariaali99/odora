...
import { DollarSign, ShoppingCart, Users, Package, AlertTriangle, ArrowUpRight, Smartphone, Globe, RefreshCw } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h2 className="text-xl sm:text-2xl font-bold font-serif text-slate-900">نظرة عامة والتحليلات البيعية</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        ...
        <div className="bg-white p-5 sm:p-6 rounded-2xl border border-slate-200 shadow-xs flex items-center justify-between">
          <div>
            ...
            <div className="text-xl sm:text-2xl font-bold text-slate-900 font-poppins mt-1">
              ...
              top_products.map((p, i) => (
                <div key={i} className="flex flex-col sm:flex-row sm:items-center justify-between p-3 rounded-xl bg-slate-50 text-xs gap-2">
                  ...
                  <div className="text-start sm:text-left pr-9 sm:pr-0">
                    <span className="font-bold text-brand-olive font-poppins">{Number(p.total_sales || 0).toFixed(2)} د.ل</span>
            ...
            {recent_orders && recent_orders.length > 0 ? (
              recent_orders.map((ord) => (
                <div key={ord.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-3 rounded-xl bg-slate-50 text-xs gap-2">
                  <div>
                    <span className="font-bold text-slate-800 font-poppins">{ord.order_number}</span>
                    <span className="text-slate-500 mr-2">{ord.customer_name} ({ord.shipping_city})</span>
                  </div>
                  <div className="flex items-center justify-between sm:justify-end gap-3">
                    <span className="font-bold text-brand-olive font-poppins">{Number(ord.total_amount).toFixed(2)} د.ل</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
            {low_stock_alerts.map((item) => (
              <div key={item.id} className="bg-white p-3 rounded-xl border border-amber-200 text-xs flex justify-between items-center shadow-2xs">
                ...
                <span className="px-2 py-0.5 rounded-full bg-red-100 text-red-700 font-bold font-poppins shrink-0">
                  باقي {item.stock} فقط
                </span>
              </div>
            ))}
