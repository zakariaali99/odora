...
import { Sparkles, Shield, Compass, Heart, Wind, Award, MapPin } from 'lucide-react';
...
export default function AboutPage() {
  ...
  return (
    <div className="py-12 md:py-20 space-y-16 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="relative rounded-3xl overflow-hidden shadow-xl aspect-[21/9] bg-odora-canvas flex items-center justify-center">
        <img 
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent flex items-end p-8 md:p-12">
          <div className="text-white max-w-xl space-y-2">
            <span className="text-xs font-mono uppercase tracking-widest text-odora-pale">ODORA A316 ARCHITECTURE</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="bg-white p-8 rounded-3xl border border-odora-dark/5 shadow-sm space-y-4">
          <div className="w-12 h-12 rounded-2xl bg-odora-canvas flex items-center justify-center text-odora-olive">
            <Wind size={24} />
          </div>
          <h3 className="font-serif font-bold text-xl text-odora-dark">
        ...
        <div className="bg-white p-8 rounded-3xl border border-odora-dark/5 shadow-sm space-y-4">
          ...
          <h3 className="font-serif font-bold text-xl text-odora-dark">
            {isRtl ? 'ضمان رسمي ودعم محلي في ليبيا' : 'Local Warranty & Support in Libya'}
          </h3>
          <p className="text-sm text-odora-dark/70 leading-relaxed">
            {isRtl 
              ? 'نضمن لك استثمارك مع ضمان رسمي لمدة عامين كاملين، بالإضافة إلى مركز صيانة معتمد وتوفر دائم لقطع الغيار والزيوت العطرية الأصلية في طرابلس وبنغازي ومصراتة.'
              : 'Enjoy peace of mind with our 2-year manufacturer warranty, dedicated Libyan service centers, and readily available parts and refill oils across all major cities.'}
          </p>
        </div>
      </div>

      {/* Libyan Heritage & Presence */}
      <div className="bg-odora-dark text-white rounded-3xl p-8 md:p-12 relative overflow-hidden">
        <div className="relative z-10 max-w-2xl space-y-6">
          <span className="inline-flex items-center gap-2 text-xs font-mono tracking-widest text-odora-pale uppercase">
            <MapPin size={14} />
            <span>{isRtl ? 'فخر الصناعة والريادة في ليبيا' : 'Designed for Libyan Living'}</span>
          </span>
          <h2 className="text-3xl md:text-4xl font-serif font-bold leading-tight">
            {isRtl ? 'حضور يغطي كافة المدن الليبية من طرابلس إلى بنغازي' : 'Present Across Libya from Tripoli to Benghazi'}
          </h2>
          <p className="text-sm md:text-base text-odora-canvas/80 leading-relaxed">
            {isRtl 
          ...
          <div className="pt-4 flex flex-wrap gap-4">
            <Link 
              to="/shop" 
              className="px-6 py-3 bg-odora-olive hover:bg-odora-olive/90 text-white rounded-xl text-sm font-semibold transition shadow-lg"
            >
              {isRtl ? 'اكتشف منتجاتنا' : 'Explore Products'}
            </Link>
            <Link 
              to="/technology" 
              className="px-6 py-3 border border-white/20 hover:bg-white/10 text-white rounded-xl text-sm font-semibold transition"
            >
