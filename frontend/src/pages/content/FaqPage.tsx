...
const DEFAULT_FAQS: Faq[] = [
  ...
  },
  {
    ...
    question_en: 'What does the 1-year warranty cover?',
    answer_ar: 'يشمل الجهاز ضماناً شاملاً لمدة عام كامل ضد أي عيوب تصنيع في المحرك الداخلي أو لوحة التحكم أو الفوهة، مع توفر قطع الغيار وخدمة الصيانة الفورية في مراكزنا بطرابلس ومصراتة.',
    answer_en: 'Our 1-year warranty covers all internal pumps, nozzles, and electronic motherboards with local servicing hubs in Tripoli and Misrata.'
  }
];
  ...
  return (
    <div className="py-12 md:py-20 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12 bg-brand-cream min-h-screen">
      ...
      <div className="text-center space-y-4">
        ...
        <p className="text-base text-brand-muted font-normal max-w-xl mx-auto">
          {isRtl 
            ? 'كل ما تود معرفته عن جهاز أودورا A316، الزيوت الفرنسية، الشحن لمدن ليبيا، والضمان.'
      ...
      <div className="relative max-w-xl mx-auto">
        <input
          ...
          className="w-full ps-10 pe-10 py-3 rounded-2xl border border-stone-200 text-sm bg-white focus:outline-none focus:border-brand-sage shadow-xs text-brand-ink"
        />
        <Search size={18} className={`absolute top-3.5 ${isRtl ? 'start-3.5' : 'end-3.5'} text-stone-400`} />
      </div>
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap transition ${
              activeCategory === cat.id
                ...
                : 'bg-white text-brand-muted hover:bg-stone-50 border border-stone-200'
            }`}
          >
      ...
      <div className="space-y-4">
        {filteredFaqs.length === 0 ? (
          <div className="p-8 text-center text-brand-muted bg-white rounded-2xl border border-stone-200 shadow-xs font-medium">
            ...
            return (
              ...
              >
                <button
                  ...
                  className="w-full p-5 text-start flex items-center justify-between gap-4 font-bold text-sm sm:text-base text-brand-ink hover:bg-stone-50 transition"
                >
                  <span>{isRtl ? faq.question_ar : (faq.question_en || faq.question_ar)}</span>
                  <ChevronDown 
                    size={18} 
                    className={`text-brand-sage shrink-0 transition-transform duration-300 ${
                      isExpanded ? 'rotate-180' : ''
                    }`} 
                  />
                </button>
                {isExpanded && (
                  <div className="px-5 pb-5 pt-1 text-xs sm:text-sm text-brand-muted font-normal leading-relaxed border-t border-stone-100">
      ...
      <div className="p-8 bg-white rounded-3xl border border-stone-200 text-center space-y-4 shadow-sm">
        <h3 className="font-bold text-lg text-brand-ink">
        ...
        <Link
          to="/contact"
          className="inline-flex items-center gap-2 px-6 py-2.5 bg-brand-sage text-white rounded-full text-xs font-bold hover:bg-brand-olive transition shadow-xs"
        >
