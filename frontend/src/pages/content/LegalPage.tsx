...
export const LegalPage: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'terms' | 'privacy' | 'warranty'>('terms');

  return (
    <div className="py-12 md:py-20 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12 bg-brand-cream min-h-screen">
      
      {/* Header */}
      <div className="text-center space-y-4">
        <h1 className="text-3xl md:text-4xl font-light text-brand-ink">
      ...
      {/* Tabs */}
      <div className="flex items-center justify-center gap-2 border-b border-stone-200 pb-3">
        <button
          onClick={() => setActiveTab('terms')}
          className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs sm:text-sm font-bold transition shadow-xs ${
            activeTab === 'terms' ? 'bg-brand-ink text-white' : 'text-brand-muted hover:bg-white border border-transparent hover:border-stone-200'
          }`}
        >
          <FileText size={16} />
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs sm:text-sm font-bold transition shadow-xs ${
            activeTab === 'privacy' ? 'bg-brand-ink text-white' : 'text-brand-muted hover:bg-white border border-transparent hover:border-stone-200'
          }`}
        >
          <Shield size={16} />
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs sm:text-sm font-bold transition shadow-xs ${
            activeTab === 'warranty' ? 'bg-brand-ink text-white' : 'text-brand-muted hover:bg-white border border-transparent hover:border-stone-200'
          }`}
        >
          <RotateCcw size={16} />
      ...
      {/* Content Body */}
      <div className="bg-white rounded-3xl p-6 sm:p-10 border border-stone-200 shadow-sm prose prose-neutral max-w-none text-brand-ink text-sm leading-relaxed space-y-6">
        
        {activeTab === 'terms' && (
          <div className="space-y-6">
            <h2 className="text-xl font-light text-brand-ink">
              {isRtl ? 'اتفاقية استخدام متجر أودورا الإلكتروني' : 'Terms and Conditions'}
            </h2>
            <p className="text-brand-muted font-normal">
              {isRtl 
            ...
            <h3 className="font-bold text-base text-brand-ink">{isRtl ? '1. الطلبات وتأكيد الشراء' : '1. Orders & Confirmation'}</h3>
            <p className="text-brand-muted font-normal">
              {isRtl
            ...
            <h3 className="font-bold text-base text-brand-ink">{isRtl ? '2. الأسعار والدفع في ليبيا' : '2. Pricing & Currency'}</h3>
            <p className="text-brand-muted font-normal">
              {isRtl
            ...
            <h3 className="font-bold text-base text-brand-ink">{isRtl ? '3. سياسة الاستخدام السليم للأجهزة' : '3. Proper Device Operation'}</h3>
            <p className="text-brand-muted font-normal">
              {isRtl
        ...
        {activeTab === 'privacy' && (
          <div className="space-y-6">
            ...
            <h3 className="font-bold text-base text-brand-ink">{isRtl ? '1. البيانات التي نجمعها' : '1. Collected Data'}</h3>
            <p className="text-brand-muted font-normal">
              {isRtl 
            ...
            <h3 className="font-bold text-base text-brand-ink">{isRtl ? '2. سرية المعلومات وعدم مشاركتها' : '2. Non-Disclosure'}</h3>
            <p className="text-brand-muted font-normal">
              {isRtl 
        ...
        {activeTab === 'warranty' && (
          <div className="space-y-6">
            <h2 className="text-xl font-light text-brand-ink">
              {isRtl ? 'سياسة الضمان والاسترجاع في ليبيا' : 'Warranty & Return Policy'}
            </h2>
            <p className="text-brand-muted font-normal">
              {isRtl 
                ? 'لأننا واثقون من جودة تصنيع أجهزة أودورا ومطابقتها للمواصفات الفندقية العالمية، نقدم لعملائنا في ليبيا ضماناً مريحاً وموثوقاً.'
                : 'We offer an industry-standard 1-year warranty and straightforward return policy across Libya.'}
            </p>

            <h3 className="font-bold text-base text-brand-ink">{isRtl ? '1. الضمان الشامل لمدة عام كامل' : '1. 1-Year Comprehensive Warranty'}</h3>
            <p className="text-brand-muted font-normal">
              {isRtl 
                ? 'يغطي الضمان محرك التذرية، الفوهة المزدوجة، اللوحة الإلكترونية، ووحدة البلوتوث ضد أي عيب تصنيعي لمدة 12 شهراً من تاريخ الشراء. في حالة حدوث أي عيب، يتم استبدال القطعة أو الجهاز عبر مركز الصيانة في طرابلس.'
                : 'Covers the atomization pump, dual nozzle, motherboard, and Bluetooth module against manufacturing defects for 12 months.'}
            </p>

            <h3 className="font-bold text-base text-brand-ink">{isRtl ? '2. حق المعاينة والاسترجاع (14 يوماً)' : '2. 14-Day Return Window'}</h3>
            <p className="text-brand-muted font-normal">
              {isRtl 
                ...
                : 'Customers have the right to inspect goods upon delivery. Unused devices in original unopened packaging may be returned within 14 days.'}
            </p>
          </div>
        )}
